...
public enum DataType {
	...
	DataType(String caption, String value, Class _class, boolean fixedFormatList, List<Format> formatList){
		...
		if (fixedFormatList) {
			...
			this.formatList = new ArrayList<Format>(formatList);
	}
	...
	/** Looks for the format by caption field */
	public Format indexOfFormatC(String caption) throws IllegalArgumentException{
		if (caption == null)
			throw new IllegalArgumentException("Ошибка при попытка поиска формата с NULL-значением");
		
		for (Format format : formatList) {
			if (format.caption.equals(caption))
				return format;
		}
	...
	/** Looks for the format by value field */
	public Format indexOfFormatV(String value) throws IllegalArgumentException{
		if (value == null)
			throw new IllegalArgumentException("Ошибка при попытка поиска формата с NULL-значением");
